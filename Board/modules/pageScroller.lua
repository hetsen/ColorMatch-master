...
-- Uppdaterad: 2013-06-10 av Marcus Thunström
--
--[[--------------------------------------------------------------------------------------
	...
	API-beskrivning:
		...
		pagination:scrollTo( pageX, pageY ) :: scrollar till angiven sida
			- pageX, pageY: Vilken sida på x- och y-axeln.

		pagination:getScrollPosition(  ) :: returnerar vilken sida som visas
			Returnerar: Horisontell och vertikal sida som visas.
...
function methods.scrollTo(scroller, x, y)
	local data = scroller[k]
	local oldX, oldY = data.x, data.y
	if x then data.x = 2-x end
	if y then data.y = 2-y end
	if data.x ~= oldX or data.y ~= oldY then
		if not data.movingStarted then scroller:dispatchEvent{name='scrollBegan', target=scroller} end
		scroller:dispatchEvent{name='scrollMoved', target=scroller, x=2-data.x, y=2-data.y}
		if not data.movingStarted then scroller:dispatchEvent{name='scrollEnded', target=scroller} end
	end
end

function methods.getScrollPosition(scroller)
	local data = scroller[k]
	return math.min(math.max(math.round(2-data.x, 1)), data.pageAmountX), math.min(math.max(math.round(2-data.y), 1), data.pageAmountY)
end
