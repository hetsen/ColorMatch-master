...
package.loaded['modules.autoLAN.modules.client'] = nil
	...
	if(UDPClient) then
		...
		if(message[2][1]==2) then
			...
			if(userMessage[1]==2) then --file transfer
				...
				if pendingFile._autolan_prevPercentReceive ~= packPercentage then
					Runtime:dispatchEvent({name = "autolanClientFileProgressReceive", fileName = filename, percent = packPercentage, nrOfPackages = userMessage[4], currentPackageNr = packetindex})
				end
				pendingFile._autolan_prevPercentReceive = packPercentage
				...
				if(pendingFile.index == userMessage[4]+1) then
					...
					----print("FILE DONE")
					Runtime:dispatchEvent({name = "autolanFileReceived", fileDir = fileDir, filename = filename})
...
local function mainLoop()		
	if(sendCredits >0) then
			...
			if(fileTable) then
				...
				local packPercentage = round(fileTable.currentPacket / fileTable.numPackets, 2)
				local fileName = fileTable.filename

				
				if fileTable._autolan_prevPercentSend ~= packPercentage then
					Runtime:dispatchEvent({name = "autolanClientFileProgressSend", fileName = fileName, percent = packPercentage, nrOfPackages = fileTable.numPackets, currentPackageNr = fileTable.currentPacket})
				end
				fileTable._autolan_prevPercentSend = packPercentage
...
local function UDPBroadcastListen()
	if(broadcastListener) then
	--print('ksdfhgksjygk,dfhjskdfjgkdsfg')
...
function client:connect(ip)
	-- tompaPrinter ("ip", ip)
	-- if ip then
	-- 	tompaPrinter ("availibleServers", availibleServers[ip])
	-- end
	-- print "<><><>"
	-- for k,v in pairs(availibleServers) do
	-- 	tompaPrinter(k,v)
	-- end
	-- print "<><><>"
	if ip and availibleServers[ip] then
		--
		stopListening()
