...
local function round(_number, _decimal)
	local mult = 10^(_decimal or 0)
 	return math.floor(_number * mult + 0.5) / mult
end

local tompaPrinter = print
local function print()
end

local fileDir = system.TemporaryDirectory
...
local applicationName = "FindTheCure-Sender"
		...
		if(message) then
			if(message[1] and message[1]=="CoronaMultiplayer" and message[2]==applicationName) then --this is the protocol id				
				if(availibleClients[clientIP] == nil) then
					...
					newPlayer.name = clientInfo.name
					...
					function newPlayer:disconnect()
						if self.clientID or UDPClients[self.clientID] then
							UDPClients[self.clientID] :close()
							UDPClients[self.clientID] = nil
						end
						local clientInfo
						if self.clientID then
							clientInfo = clientsInfo[self.clientID]
						end
						if clientInfo then
					...
					Runtime:dispatchEvent({name = "autolanPlayerJoined", clientIP = clientIP, clientID = numClients, client = newPlayer})
		...
		for i,pendingConnection in pairs(pendingConnections) do
			...
			if(msg and clientIP==pendingIP and port == pendingPort) then --make sure the packet arrives from the right place
				...
				if(message and message[1] == "e") then	
					...
					Runtime:dispatchEvent({name = "autolanPlayerJoined", clientIP = clientIP, clientID = numClients, client = newPlayer, internet= true})
				end
			end
		end		
				...
				if(message) then
					...
					if(userMessage[1]==2) then --file transfer
						...
						if(pendingFiles[filename]==nil) then
							pendingFile = {}
							pendingFile.file = io.open(system.pathForFile(userMessage[2],fileDir),"wb")
						...
						pendingFile = pendingFiles[filename]		
						local packetindex = userMessage[3]


						local packPercentage = round(packetindex / userMessage[4], 2)

						local index = table.indexOf(UDPClients, UDPClient)
						local currentClient
						if index then
							if clientsInfo[index] then
								currentClient = clientsInfo[index].clientObject
							end
						end

						if currentClient then
							if currentClient._autolan_prevPercentReceive ~= packPercentage then
								Runtime:dispatchEvent({name = "autolanServerFileProgressReceive", client = currentClient, fileName = filename, percent = packPercentage, nrOfPackages = userMessage[4], currentPackageNr = packetindex})
							end
							currentClient._autolan_prevPercentReceive = packPercentage
						end
						...
						if(pendingFile.index == userMessage[4]+1) then
							...
							----print("FILE DONE")
							Runtime:dispatchEvent({name = "autolanFileReceived", fileDir = fileDir, filename = filename, clientID = i})
...
local function mainLoop() --handles both data transfer, ACKS, disconnecting clients
	...
	-------------------------files	
		...
		if(fileTable) then	
			...
			--tompaPrinter("packTest", fileTable.currentPacket.."/"..fileTable.numPackets)
			
			local currentPackNr, numPackages = fileTable.currentPacket, fileTable.numPackets - 2
			local packPercentage = round(currentPackNr / numPackages, 4)
			local fileName = fileTable.filename


			local index = (type(fileQueueClients) == "table" and fileQueueClients[1])
			local currentClient
			if index then
				if clientsInfo[index] then
					currentClient = clientsInfo[index].clientObject
			...
			if currentClient then
				if currentClient._autolan_prevPercentSend ~= packPercentage then
					Runtime:dispatchEvent({name = "autolanServerFileProgressSend", client = currentClient, fileName = fileName, percent = packPercentage, nrOfPackages = fileTable.numPackets, currentPackageNr = fileTable.currentPacket})
				end
				currentClient._autolan_prevPercentSend = packPercentage
			end
			if(data) then
